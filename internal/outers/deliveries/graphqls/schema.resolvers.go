...
func (r *queryResolver) Account(ctx context.Context, id string) (*model.Account, error) {
	if id == "simulate-problem" {
		// Simulate a CPU/Memory spike and delay, capturing it in tracing
		done := make(chan bool)
		go func() {
			// CPU Burn + Memory Allocation simulation
			var memoryHog [][]byte
			for i := 0; i < 100; i++ {
				memoryHog = append(memoryHog, make([]byte, 1024*1024*2)) // 2MB * 100 = 200MB
			}
			for i := 0; i < 1000000; i++ {
				_ = i * i
			}
		...
		return nil, fmt.Errorf("simulated problem encountered on Account fetching")
	}
